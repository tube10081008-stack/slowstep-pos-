...
| `SOLAPI_API_KEY` | (선택) 문자 발송. 없으면 Mock — 실제로 나가지 않는다 |
| `SOLAPI_API_SECRET` | 위와 한 쌍. 서명에만 쓰이고 어디에도 실려 나가지 않는다 |
| `SMS_SENDER_PHONE` | 발신번호. **솔라피에 사전 등록한 번호**여야 한다 |
| `SMS_OPT_OUT_NUMBER` | 광고성 문자 무료수신거부 번호(법적 표기) |

> **문자 발송 준비 순서** — 솔라피(solapi.com) 가입 → **발신번호 사전등록**
> (통신서비스 이용증명원 필요, 승인에 1~2일) → 잔액 충전 → API Key/Secret 발급.
> 발신번호 등록 없이 키만 넣으면 발송이 통째로 거부된다.
>
> 세 값이 다 들어가야 실제 발송이다. **하나라도 빠지면**
> `/api/v1/health` 의 `config.sms` 가 `partial` 로 뜨고 대시보드에도 경고가
> 뜬다(`mock` = 아직 실제로 안 나감, `live` = 진짜 나감).
>
> 광고성 문자는 **08시~21시**에만 나간다(정보통신망법 제50조 제3항).
> 페이히어에서 받은 마케팅 동의는 **승계되지 않으므로**, 매장에서 다시 동의를
> 받은 분에게만 광고성 발송이 나간다. 정보성(쿠폰 발행·포인트 안내)은
> 동의 없이도 보낼 수 있고 시간 제한도 없다.
