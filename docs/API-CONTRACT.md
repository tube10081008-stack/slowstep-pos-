...
##### 분할 결제 — `POST /api/v1/transactions` 의 `split_method` · `split_amount`
현금 얼마 + 카드 나머지처럼 손님이 나눠 낼 때. **`split_amount` 를 보조 수단이
받고, 나머지(`net_amount − split_amount`)를 `payment_method` 가 받는다.**

- 어긋난 값은 **거절이 아니라 분할 없음**으로 떨어뜨린다(수기 할인과 같은 판단).
  보조 수단이 비었거나 / 주 결제수단과 같거나 / 금액이 0 이하이거나 /
  실결제액 이상이면 분할로 기록하지 않는다.
- **정산의 수단별 집계(`by_method`)는 이 값을 둘로 쪼개 넣는다.** 한쪽에 몰아
  잡으면 카드 단말기 합계와 안 맞고, 거기서 정산이 틀어지기 시작한다.
- 결제수단을 표로 분리하지 않고 두 칸으로 둔 이유: 카페에서 셋으로 쪼개는 일은
  없고, 이 값을 읽는 곳이 수단별 집계 한 군데뿐이다.

##### 세트 할인 — `POST /api/v1/transactions` 의 `set_discount`
음료+디저트를 함께 주문했을 때 `min(음료수, 대상 디저트수) × Store.set_discount_amount`
만큼 깎는다. **기본값은 꺼짐이고, 직원이 POS에서 눌렀을 때만 적용된다.**
자동으로 먹이면 손님도 직원도 모르는 채 할인이 나가고, 안 깎아도 될 주문까지
깎인다. 조건이 안 되면(디저트가 빠지면) 값을 보내도 0원이다.

**대상 디저트는 `MenuItem.set_eligible` 로 정한다.** 확정 메뉴판이 '음료 +
플레인 휘낭시에'로 못박아서, 플레인 휘낭시에에만 켜 둔다(마이그레이션 0031).
메뉴판 수정 페이지의 디저트 수정 창에서 켜고 끈다.

- **켜진 메뉴가 하나도 없으면 예전처럼 디저트 전체**를 친다. 실수로 다 꺼
  두면 세트 할인이 조용히 죽어 버리는데, 그건 계산대에서 알아채기 어렵다.
- 판매중지된 메뉴는 '켜진 것'으로 세지 않는다 — 대상 디저트를 내려 두면
  남은 디저트로 세트가 돌아간다.
...
주문 내역(주문 항목·회원명·수단 포함). **결제 시각(`paid_at`) 내림차순.**
기본 정렬(`-created_at`)로 주면 POS가 날짜로 묶을 때 같은 날짜 머리가 여러 번
뜬다 — 승인 전 상태로 있다가 나중에 확정된 건에서 둘이 갈린다.

- `date`를 주면 **그날 결제된 건 전부**(`paid_at` 기준, 매장 시간대).
- 비우면 날짜 무관 **최근 100건**. `sales/summary`와 같은 규칙으로, 형식이
  틀린 `date`는 무시하고 최근 목록을 준다.

### `GET /api/v1/integrity?limit=10`  🔒
**데이터 정합성 점검** — 돈과 관련된 불변식이 깨졌는지 본다.
서버리스에는 셸이 없어 `manage.py check_integrity` 를 못 돌리므로, 운영 DB를
확인할 통로로 API를 열어 둔다.
```json
200 { "ok": true, "total_problems": 0,
      "counts": { "members": 46, "transactions": 16, ... },
      "checks": [ { "name": "포인트 잔액 vs 원장", "bad": 0, "sample": [], "hint": "…" }, … ] }
```
검사 항목 — 포인트 잔액 vs 원장 / 거래 금액(`net = gross − discount − points_used`) /
분할 결제 금액 / 주문 항목 합계 / 누적결제·방문수 / 쿠폰 사용 표시 /
결제완료인데 `paid_at` 없음 / 주문번호 중복 / 잔액 음수.

> **버그가 아니라 손해를 잡는 검사다.** 잔액이 원장과 어긋나면 손님에게 줄
> 돈이 틀어지고, 거래 금액이 어긋나면 정산이 틀어진다. 조용히 틀어진 채로
> 굴러가는 게 가장 위험하다.

### `GET /api/v1/export?kind=members|transactions|points|coupons`  🔒
**데이터 내보내기(백업)** — CSV. 엑셀에서 바로 열리도록 UTF-8 BOM을 붙인다.
관리형 DB의 백업을 믿되, **믿는 것과 사본을 갖는 것은 다르다.**
특히 `points`(포인트 원장)는 잔액의 진실 원천이라 이것만 있어도 복원할 수 있다.

### `POST /api/v1/points/grant`  🔒
**수기 포인트 지급·회수** — `{ "member_id": 1, "delta": 1000 }` (회수는 음수).
잔액만 만지지 않고 **원장(`PointEntry`, `adjust`)에 함께 남긴다.** 지급 경로를
하나로 모아 두지 않으면 잔액과 내역이 갈라진다(정합성 점검기가 잡는 어긋남).
보유 포인트보다 많이 회수하려 하면 400.

### 프로모션 — `GET /api/v1/promos` (공개) · `POST`·`PATCH`·`DELETE` 🔒
메뉴판 사이에 한 장씩 끼워 도는 광고. 신메뉴 안내·휴무 공지·이벤트용.

- **읽기는 공개**다 — 고객 화면(메뉴판)이 직접 부른다. 메뉴·매장 정보와 같은
  성격이라 토큰을 요구하면 손님 화면이 먹통이 된다. **쓰기는 막는다.**
- 손님 화면에는 `is_active=true` 인 것만 내려간다. 점주 화면(토큰 있음)에는
  꺼 둔 것도 함께 내려간다 — 안 보이면 다시 켤 수가 없다.
- 이미지 상한은 **base64 90만 자(약 675KB)** 로 메뉴 썸네일(40만 자)보다 크다.
  화면 한 장을 채우므로 900px 로는 흐리다(화면에서 1600px·JPEG 0.82 로 줄여 올린다).
- 새로 올린 것은 **맨 뒤**에 붙는다(`sort_order`). 기존 순서를 밀고 들어오면
  손님이 보던 차례가 뒤집힌다.
- 메뉴판은 메뉴 페이지를 다 보여준 뒤 프로모션을 한 장씩 돌린다. 탭 이름은
  `이벤트 1·2`로 고정이고 **`title` 을 쓰지 않는다** — 그건 관리용 이름이라
  파일명("promo1")이 그대로 들어가 있을 수 있다.
- **프로모션 장은 화면을 통째로 쓴다**(`body.full`) — 머리말·발문·여백을
  걷어내고 이미지만 남긴다. 그래야 포스터 글씨가 읽힌다. 메뉴 장으로
  돌아오면 그대로 복구된다. 넘김 버튼은 남긴다(없으면 그 장에 갇힌다).
- 남는 자리는 **같은 그림을 크게 흐려 깐다**(`.promo .bg`). 흰 띠가 남으면
  액자에 끼운 것처럼 옹색해 보이고, `cover` 로 꽉 채우면 세로 포스터의 가격이
  잘려 나간다. 흐린 배경을 깔면 자르지 않고도 화면이 꽉 찬다.
- 넘김 버튼은 **평소엔 감춰 둔다.** 아무도 안 만지는 붙박이 화면에 동그라미가
  그림 위에 늘 떠 있으면 그게 제일 먼저 눈에 띈다. 마우스·터치·키 입력이
  있으면 2.6초 나타난다(`body.full.hint`).
- 화면이 1920 폭이라 업로드도 **1920px** 로 줄인다. 그보다 작게 줄이면
  포스터 글씨가 뭉개진다.
- `MenuItem` 과 분리한 이유: 파는 물건이 아니라 한 장짜리 그림이다.
  메뉴에 끼워 넣으면 가격·옵션·원가가 딸려 오고 메뉴 목록에도 섞인다.

### 메뉴판 사진·노출 — `MenuItem.image` · `MenuItem.show_on_board`
`/menuboard/` (메뉴판 수정 페이지)가 쓰는 두 필드. 둘 다 `GET /api/v1/menu` 로
내려가고 `PATCH /api/v1/menu/{id}` 로 고친다.

- **`image` 는 data URI 를 DB에 그대로 담는다.** 서버리스에는 쓸 수 있는
  디스크가 없고, S3·클라우디너리를 붙이면 계정과 키가 하나 더 는다.
  브라우저에서 가로 900px·JPEG 0.72 로 줄여 올리므로 한 장 60~120KB,
  메뉴 30종이면 3MB 남짓이라 Neon 무료(0.5GB)로 충분하다.
  서버도 상한(base64 40만 자 ≈ 300KB)과 `data:image/` 접두어를 따로 검사한다 —
  화면 코드를 거치지 않고 부를 수 있다. 빈 문자열을 보내면 사진 삭제.
- **`show_on_board` 는 고객 메뉴판 노출만 끈다. POS에서는 그대로 팔린다.**
  파는 것과 보여주는 것은 다르다 — 디저트는 매일 바뀌고, 특정 손님 전용
  메뉴(레온·킹)는 붙박이 메뉴판에 걸면 안 된다.
- 예전에는 이 규칙이 `web/display/menu.html` 안에 낱말 목록으로 박혀 있었다.
  메뉴 하나 숨기려고 코드를 고쳐 배포하는 건 사장님이 할 수 있는 일이 아니라
  DB로 옮겼다(마이그레이션 0028 이 그때 숨겨져 있던 것을 그대로 옮긴다).
  메뉴판 화면은 이 값이 **없을 때만** 옛 낱말 규칙으로 떨어진다 — 배포 순서가
  엇갈려 서버가 아직 새 필드를 안 내려줄 때 메뉴판이 갑자기 다 보이면 안 된다.

### `POST /api/v1/campaigns/compose`  🔒
**문자 문구 자동 생성** — `{ "purpose": "comeback", "benefit": "아메리카노 1+1",
"tone": "warm", "is_ad": true, "long_form": false }`.
→ `{ "text": "...", "source": "gemini"|"rule", "bytes": 54, "budget": 56, "type": "SMS" }`

- `purpose` — `comeback` / `new_menu` / `coupon` / `points` / `thanks` / `notice`.
- `GEMINI_API_KEY` 가 없으면 **규칙 기반 초안**으로 떨어진다(`source: "rule"`).
  AI가 없다고 기능이 멈추면 안 된다(ai_order.py 와 같은 2단 구조).
- **모델 출력을 그대로 믿지 않는다.** 문자는 되돌릴 수 없고 글자수가 곧 요금이다:
  `(광고)` 표기·수신거부 안내·전화번호·링크는 걷어내고(발송 직전에 시스템이
  붙이므로 두 번 들어간다), 우리가 모르는 치환 변수는 지우며(치환 안 된 채
  손님에게 간다), 길이는 서버가 다시 재서 문장 끝에서 자른다.
- 광고성 SMS 의 본문 예산은 **56바이트**다(`(광고)` + 수신거부가 34바이트를
  먼저 먹는다). 예산이 빠듯하면 **입력한 혜택을 먼저 세우고** 인사말을 버린다 —
  문자의 핵심은 혜택이다.

### 대상 직접 고르기 — `Campaign.recipients`
`POST /api/v1/campaigns` 에 `recipients: [회원id, ...]` 를 실으면 **그 사람들에게만**
나간다(세그먼트는 기록용으로만 남는다). 비우면 예전처럼 세그먼트로 대상을 뽑는다.

- 세그먼트만으로는 "이 사람은 빼고"가 안 된다. 계산대에서 아는 사정은 필터로
  표현되지 않는다.
- **골랐다고 수신 동의 확인을 건너뛰지 않는다.** 광고성이면 미동의자는 골라도
  `skipped`(마케팅 수신 미동의)로 제외된다 — 과태료 대상이라 화면 선택이
  이걸 덮을 수 없어야 한다.
- `POST /api/v1/segments/preview` 가 `members[]` 로 **전체 명단**을 함께 준다
  (체크박스로 고르려면 앞 10명만 봐서는 못 고른다). 각 줄에 `marketing_opt_in`
  과 `has_phone` 이 있어 화면이 '왜 안 가는지'를 미리 보여 준다.

### `POST /api/v1/sms/test`  🔒
**테스트 발송** — `{ "phone": "01012345678", "text": "..." }`.
실전 첫 발송이 곧바로 세그먼트 전체(42명)로 나가면 되돌릴 수 없다. 발신번호
등록·잔액·키가 다 맞는지 **본인 폰 한 대로** 먼저 확인하는 통로다.

- 항상 **정보성**으로 나간다((광고) 표기 없음) — 광고성 테스트는 야간 제한과
  동의 확인에 걸려 테스트가 안 된다.
- `MessageLog` 를 남기지 않는다. 캠페인 발송 통계가 테스트로 오염되면
  '몇 명에게 실제로 나갔나'를 못 믿는다.
- 키가 없으면 `mocked: true` 로 성공을 돌려준다(실제로 안 나감).
- 대행사 오류는 **502** + `detail`.

### `POST /api/v1/coupons/grant`  🔒
**수기 쿠폰 발행** — `{ "member_id": 1, "kind": "bogo", "note": "매장 지급" }`.
`kind` 는 `bogo` / `free_drink` / `discount_10` / `discount_20` / `beans_200`.

- 발행 사유는 **`manual`(수기 지급)** 로 남는다 — 룰렛·등급 승급·랭킹으로
  나간 자동 발행분과 섞이면 나중에 쿠폰 비용을 갈라 볼 수 없다.
- 만료는 발행 시각 + **90일**(`Coupon.DEFAULT_VALID_DAYS`)이 자동으로 붙는다.
- **포인트 원장은 건드리지 않는다.** 쿠폰은 잔액이 아니라 장 단위라
  `PointEntry` 에 섞지 않는다(원가 성격이 다르다).
- 응답에 발행된 쿠폰과 그 회원의 현재 보유 목록(`coupons`)을 함께 싣는다 —
  계산대에서 두 번 주는 실수를 막기 위함.

### `POST /api/v1/prepaid`  🔒
**선결제(충전)** — `{ "member_id": 1, "amount": 30000, "payment_method": "CARD" }`.
낸 금액을 **그대로 포인트로** 넣는다.

- **3% 적립은 붙지 않는다.** 충전 자체가 포인트를 주는 행위라 여기에 적립까지
  얹으면 이중이다(3만원 충전에 30,900P가 나간다).
- 매출·누적결제액·방문수·등급에는 정상 반영된다 — 실제로 낸 돈이다.
- 회원에게만 가능(비회원 400). 포인트 사용·쿠폰·할인과 섞이지 않는다.
- **취소하면 넣어 준 포인트를 함께 회수한다.** 취소 로직은 원래
  `사용분 − 적립분`만 되돌렸는데, 충전은 둘 다 0이라 **돈만 환불되고
  포인트는 남아 있었다.** 이 거래에 묶인 `adjust` 원장을 충전분으로 보고
  같이 되돌린다(수기 지급은 거래에 묶이지 않아 여기 잡히지 않는다).
- 충전은 스탬프를 주지 않으므로 **취소가 스탬프를 빼지도 않는다.**
