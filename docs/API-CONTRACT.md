...
```json
{ "key": "taste:dessert", "kind": "taste", "title": "디저트 처음 만나기",
  "description": "플레인 휘낭시에 어떠세요?", "progress": 0, "target": 1,
  "reward": 500, "is_completed": false }
```
- `kind` — `comeback`(평소 주기의 1.5배 넘게 안 오심) / `collection`(1~2종 남은
  카테고리) / `taste`(미경험 카테고리) / `rhythm`(이번 주) / `stretch`(개인 월
  방문 기준선 ×1.2) / `option`(오트·디카페인·샷) / `timeslot`(오전·저녁).
- `stretch`의 목표는 **최근 3개월 개인 평균**에서 계산한다 — 주 3회 오는 손님과
  월 1회 오는 손님에게 같은 숫자를 들이밀지 않는다.
- 보상은 퀘스트당 **2,000P 상한**. 적립비용은 마진 대시보드에 그대로 반영된다.
- 지급은 결제 시점에 서버에서 판정하며, 응답 `rewards[]`에 `{"type":"quest"}`로
  실린다(고객 화면이 바로 띄운다).
